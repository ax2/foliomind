...
impl WebHost {
    pub fn start(app: AppHandle, host: PiHost) -> Result<Self, String> {
        ...
        Ok(Self { address, shutdown, join: Some(join) })
    }
...
fn handle_connection(mut stream: TcpStream, app: &AppHandle, host: &PiHost, token: &str) {
    ...
    if request.method == "OPTIONS" {
        if origin.as_deref().is_some_and(origin_allowed) {
            ...
            write_json(&mut stream, None, 403, json!({"error": "origin is not allowed"}));
        }
        return;
    }
    if request.path == "/api/health" && request.method == "GET" {
        write_json(&mut stream, origin.as_deref(), 200, json!({"ok": true, "service": "foliomind-local-host"}));
        return;
    }
    if !origin.as_deref().is_some_and(origin_allowed) {
        write_json(&mut stream, None, 403, json!({"error": "origin is not allowed"}));
    ...
    if request.path == "/api/session" && request.method == "GET" {
        write_json(&mut stream, origin.as_deref(), 200, json!({"token": token, "service": "foliomind-local-host"}));
        return;
    }
    if request.headers.get("x-foliomind-host").map(String::as_str) != Some(token) {
        write_json(&mut stream, origin.as_deref(), 401, json!({"error": "invalid local host session"}));
        return;
    }
    ...
    match result {
        ...
        Err((status, error)) => write_json(&mut stream, origin.as_deref(), status, json!({"error": error})),
    }
    let _ = stream.shutdown(Shutdown::Both);
}

fn route_request(request: &HttpRequest, app: &AppHandle, host: &PiHost) -> Result<Value, (u16, String)> {
    match (request.method.as_str(), request.path.as_str()) {
        ("GET", "/api/integration/status") => {
            let status = IntegrationStatus {
                credential_configured: host.credentials.read_qveris_key().map_err(internal_error)?.is_some(),
        ...
        ("POST", "/api/integration/credential") => {
            ...
            host.credentials.write_qveris_key(&input.api_key).map_err(internal_error)?;
            host.discard_staged_model_catalog(None);
            Ok(json!({"configured": true}))
        }
        ("DELETE", "/api/integration/credential") => {
            host.credentials.delete_qveris_key().map_err(internal_error)?;
            host.discard_staged_model_catalog(None);
            Ok(json!({"configured": false}))
        }
        ("POST", "/api/integration/models/sync") => {
            let envelope: InputEnvelope<SettingsInput> = parse_body(request).map_err(client_error)?;
            serde_json::to_value(sync_model_catalog(host, app, envelope.input).map_err(internal_error)?).map_err(internal_error)
        }
        ("POST", "/api/integration/settings") => {
            let envelope: InputEnvelope<SettingsInput> = parse_body(request).map_err(client_error)?;
            serde_json::to_value(apply_integration_settings(host, app.clone(), envelope.input).map_err(internal_error)?).map_err(internal_error)
        }
        ("GET", "/api/user-state") => serde_json::to_value(user_state::load(app).map_err(internal_error)?).map_err(internal_error),
        ("POST", "/api/user-state") => {
            let envelope: StateEnvelope = parse_body(request).map_err(client_error)?;
            serde_json::to_value(user_state::save(app, &envelope.state).map_err(internal_error)?).map_err(internal_error)
        }
        ("GET", "/api/runtime/status") => serde_json::to_value(host.status()).map_err(internal_error),
        ("POST", "/api/runtime/start") => serde_json::to_value(host.start(app.clone()).map_err(internal_error)?).map_err(internal_error),
        ("POST", "/api/runtime/stop") => serde_json::to_value(host.stop(app.clone()).map_err(internal_error)?).map_err(internal_error),
        ("POST", "/api/runtime/abort") => {
            let response = host.request(json!({"type": "abort"}), Duration::from_secs(5)).map_err(internal_error)?;
...
fn run_prompt(host: &PiHost, app: &AppHandle, input: PromptInput) -> Result<Value, (u16, String)> {
    ...
    if matches!(host.status().state, super::RuntimeState::Stopped | super::RuntimeState::Crashed) {
    ...
    ensure_rpc_success(&response)?;
    let timeout = Duration::from_millis(input.timeout_ms.unwrap_or(PROMPT_TIMEOUT.as_millis() as u64).clamp(1_000, 125_000));
    ...
    loop {
        ...
        let event = receiver.recv_timeout(remaining).map_err(|error| match error {
            std::sync::mpsc::RecvTimeoutError::Timeout => (504, "Pi 分析等待超时".into()),
            std::sync::mpsc::RecvTimeoutError::Disconnected => (502, "本地 Host 事件通道已关闭".into()),
        })?;
            ...
            if let Some(message) = frame.get("message") {
                if message.get("role").and_then(Value::as_str) == Some("assistant") {
                    ...
                    if message.get("stopReason").and_then(Value::as_str) == Some("error") {
                        terminal_error = message.get("errorMessage").and_then(Value::as_str).map(str::to_owned);
                    }
                }
            }
        }
...
fn ensure_rpc_success(response: &Value) -> Result<(), (u16, String)> {
    if response.get("success").and_then(Value::as_bool) == Some(false) {
        return Err((409, response.get("error").and_then(Value::as_str).unwrap_or("Pi 拒绝了请求").into()));
    }
    Ok(())
}
...
fn read_request(stream: &mut TcpStream) -> Result<HttpRequest, String> {
    ...
    let header_end = loop {
        ...
        let read = stream.read(&mut chunk).map_err(|_| "无法读取本地 Host 请求".to_string())?;
        if read == 0 { return Err("请求提前关闭".into()); }
        buffer.extend_from_slice(&chunk[..read]);
        if buffer.len() > MAX_HEADER_BYTES { return Err("请求头过大".into()); }
        if let Some(index) = buffer.windows(4).position(|window| window == b"\r\n\r\n") { break index; }
    };
    let header_bytes = &buffer[..header_end];
    let header_text = std::str::from_utf8(header_bytes).map_err(|_| "请求头不是 UTF-8".to_string())?;
    ...
    let content_length = headers.get("content-length").map_or(Ok(0), |value| value.parse::<usize>().map_err(|_| "Content-Length 无效"))?;
    if content_length > MAX_BODY_BYTES { return Err("请求体过大".into()); }
    let body_start = header_end + 4;
    while buffer.len() < body_start + content_length {
        let mut chunk = [0_u8; 4096];
        let read = stream.read(&mut chunk).map_err(|_| "无法读取请求体".to_string())?;
        if read == 0 { return Err("请求体提前关闭".into()); }
        buffer.extend_from_slice(&chunk[..read]);
    }
    Ok(HttpRequest { method, path, headers, body: buffer[body_start..body_start + content_length].to_vec() })
}

fn origin_allowed(origin: &str) -> bool {
    let Ok(url) = url::Url::parse(origin) else { return false };
    if url.scheme() != "http" || url.username() != "" || url.password().is_some() || url.path() != "/" && !url.path().is_empty() { return false; }
...
fn write_json(stream: &mut TcpStream, origin: Option<&str>, status: u16, body: Value) {
    let bytes = serde_json::to_vec(&body).unwrap_or_else(|_| b"{\"error\":\"response encoding failed\"}".to_vec());
    write_response(stream, origin, status, "application/json; charset=utf-8", &bytes);
}

fn write_response(stream: &mut TcpStream, origin: Option<&str>, status: u16, content_type: &str, body: &[u8]) {
    let reason = match status { 200 => "OK", 204 => "No Content", 400 => "Bad Request", 401 => "Unauthorized", 403 => "Forbidden", 404 => "Not Found", 409 => "Conflict", 500 => "Internal Server Error", 502 => "Bad Gateway", 504 => "Gateway Timeout", _ => "Error" };
    let cors = origin.filter(|value| origin_allowed(value)).map_or(String::new(), |value| format!("Access-Control-Allow-Origin: {value}\r\nVary: Origin\r\n"));
