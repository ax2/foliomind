...
- 市场行情总览与跨市场自选列表，缺失数据保持为空；支持核心估值、交易盘面、完整字段和通过 canonical workspace 跨端恢复的自定义命名行情列视图。
...
- 研究筛选支持涨跌幅、PE、PB、成交量等真实数值条件，可与文本/方向筛选组合并通过 canonical workspace 保存最多 10 个命名预设；缺失字段不会被猜测或补值。
- 研究筛选完成完整真实行情后可导出当前结果 CSV；未配置、部分返回或缺失行情时保持导出禁用，导出不含凭据或原始运行数据。
- 研究筛选的完整真实结果可选择最多 4 个标的进行本地横向对比；缺失字段保持为空，不发起额外请求或写入用户状态。
- 研究结果可直接打开预填标的的盯盘编辑器；必须显式保存后才创建规则，不复制行情快照或绕过真实数据门禁。
- 组合持仓可直接回到对应标的的行情证据抽屉；入口只读真实行情来源和字段覆盖，不修改持仓或交易计划。
- 盯盘编辑器支持保存、应用、复制和删除个人条件模板；模板只保存受限条件，不包含行情、凭据或触发历史。
...
首次打开且本机没有 FolioMind 状态时，应用会先提供“从空工作区开始”和“导入已有状态”两条路径；选择前不会展示或保存内置自选。已有设备可从设置导出的 JSON 备份导入，导入会同时恢复自选与工作区偏好。
...
完成首次工作区选择后，即使未配置凭证和模型，界面也只显示可操作的真实空态或等待配置状态；一旦配置完成，行情、指标、图表、组合和盯盘信号只使用数据服务已返回的真实数据，缺失字段显示为空并提示查询，不会用样例补齐。设置页可分别测试已保存的数据连接和模型连接；模型探针只发起一次最小请求，不调用金融工具，但可能产生模型费用。
...
GitHub Actions 的 `release` workflow 会在 `main` 的版本提交后自动运行，也支持手工触发；版本从 `package.json`、Cargo 和 Tauri 配置一致性校验中读取（当前为 `0.1.235`）。若同一版本已经正式发布，后续同版本提交会在准备阶段安全跳过，不重复构建或覆盖资产。发布会先完成格式、严格 Clippy、测试以及隔离 Web/Local Host Playwright 回归，再构建 Windows NSIS/MSI 和 macOS Apple Silicon DMG，并在对应 runner 上执行安装/卸载或 DMG 挂载复制烟测；确认三类安装包齐全并通过 SHA-256 校验后，才创建或复用 `v<version>` draft release、上传安装包与 `SHA256SUMS.txt` 并正式发布。Windows 安装包使用稳定的 WiX UpgradeCode、禁止降级并采用 current-user 安装模式；可识别的同一产品新版本会直接覆盖升级，不要求用户先手动卸载或重复确认，只有无法识别为同一产品时才保留系统安全确认。配置、API Key 和用户数据位于安装目录之外，会保留在升级后。
