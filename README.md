...
- 自选股分组、分组筛选与按名称/最新价/涨跌幅排序；缺失行情始终保持为空，侧栏会显示真实报价的新鲜度、来源和交易所时区；长名称会安全省略并保留完整悬浮提示；未知市场使用明确的 UTC 参考而不会猜测本地时区；支持 FolioMind CSV/TradingView 风格 TXT 的批量导入导出。
