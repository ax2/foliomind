...
3. 点击“同步模型”，从网关的 `/models` 读取当前可用模型，再选择 Pi 默认模型并保存应用。
...
GitHub Actions 的 `release` workflow 接收与仓库配置一致的 SemVer（当前为 `0.1.0`），创建或复用 `v<version>` draft release，分别构建 Windows NSIS/MSI 和 macOS Apple Silicon DMG。所有平台成功后，workflow 会上传安装包与 `SHA256SUMS.txt` 到 Release assets 并正式发布。
