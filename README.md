...
- 自选股分组、分组筛选与按名称/最新价/涨跌幅排序；缺失行情始终保持为空，侧栏会显示真实报价的新鲜度/可能延迟状态；支持 FolioMind CSV/TradingView 风格 TXT 的批量导入导出。
