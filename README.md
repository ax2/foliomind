...
GitHub Actions 的 `release` workflow 只允许从当前 `main` 提交运行，并接收与仓库配置一致的 SemVer（当前为 `0.1.0`）。它会创建或复用 `v<version>` draft release，分别构建 Windows NSIS/MSI 和 macOS Apple Silicon DMG；格式、严格 Clippy、测试与所有平台构建成功后，workflow 才会上传安装包与 `SHA256SUMS.txt` 并正式发布。
