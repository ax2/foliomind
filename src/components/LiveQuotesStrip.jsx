...
export function LiveQuotesStrip() {
  ...
  return <section className="live-quotes-strip" aria-label="实时行情"><div className="live-quotes-heading"><strong>实时行情</strong><span>{liveDataLoading ? "正在更新…" : "每分钟自动更新 · 仅显示真实数据"}</span></div><div className="live-quotes-grid">{watchlist.map((item) => { const quote = liveQuotes[item.symbol]; const hasQuote = Number.isFinite(quote?.price); return <article key={item.symbol}><div><strong>{item.name}</strong><small>{item.symbol}</small></div><b>{hasQuote ? formatPrice(quote.price) : "—"}</b><span className={quote?.change >= 0 ? "up" : "down"}>{Number.isFinite(quote?.change) ? formatPercent(quote.change) : hasQuote ? "涨跌幅暂无" : "等待数据"}</span>{hasQuote && <small>{quote.source || "数据服务"}{quote.asOf ? ` · ${quote.asOf}` : ""}</small>}</article>; })}</div>{liveDataError && <p className="live-quotes-error" role="status">部分标的暂未更新，系统会稍后自动重试。</p>}</section>;
}
