...
export function StockWorkspace() {
  ...
  const healthTitle = { checking: "正在读取数据连接", "connection-error": "数据连接暂不可用", preview: "等待配置真实数据", loading: "正在获取真实行情", fresh: "真实行情 · 数据新鲜", stale: "真实行情 · 可能已延迟", unknown: "真实行情 · 数据时间未知", error: "暂未获取到行情", empty: "等待真实行情" }[healthState];
  ...
  return (
    <main className="stock-workspace">
      ...
      <div className={`primary-price ${changeToneClass(change)}`}>{price == null ? "—" : formatPrice(price)} <span>{change == null ? (integrationStatusLoading ? "正在读取数据连接" : integrationStatusError ? "数据连接暂不可用" : realDataMode ? "尚未查询真实行情" : "等待配置真实数据") : `${changeAmount == null ? "" : `${changeAmount >= 0 ? "+" : ""}${formatPrice(changeAmount)}　`}${formatPercent(change)}`}</span><small className={hasQuote ? `quote-source quote-source-${freshness.state}` : ""}>{hasQuote ? `数据源 · ${quote.source || "真实数据"} · ${formatQuoteFreshness(quote.asOf, Date.now(), stock.market)}` : integrationStatusLoading ? "连接确认后才会显示真实数据" : integrationStatusError ? "请到设置检查本地 Host" : realDataMode ? liveDataLoading ? "正在查询真实行情" : "暂无已查询数据 · 点击实时数据获取" : "配置数据服务后显示真实行情"}</small></div>
