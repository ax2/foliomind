...
export function WatchlistSidebar() {
  ...
  return <aside className="watchlist-sidebar">
    ...
    <div className="watch-groups">{watchlist.map((item) => { const quote = liveQuotes[item.symbol] || (previewMode ? stocks[item.symbol] : null); const hasQuote = Number.isFinite(quote?.price); const hasChange = Number.isFinite(quote?.change); return <div className={selectedSymbol === item.symbol ? "watch-row selected" : "watch-row"} key={item.symbol}><button className="watch-row-main" onClick={() => selectSymbol(item.symbol)}><span><strong>{item.name}</strong><small>{item.symbol}</small></span><span className={`quote${hasChange ? quote.change >= 0 ? " up" : " down" : ""}`}><strong>{hasQuote ? quote.price.toFixed(2) : "—"}</strong><small>{hasChange ? `${quote.change >= 0 ? "+" : ""}${quote.change.toFixed(2)}%` : realDataMode ? "尚未查询" : previewMode ? "预览模式" : "等待配置"}</small></span></button><button className="watch-remove" aria-label="移除自选" onClick={() => { void removeWatchlist(item.symbol); }}><X size={12} /></button></div>; })}</div>
