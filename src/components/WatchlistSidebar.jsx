...
export function WatchlistSidebar() {
  ...
  return <aside className="watchlist-sidebar">
    ...
    <div className="watch-groups">{groups.map(([label, items]) => <section key={label}><h3>{label}<CaretDown size={13} /></h3>{items.map((item) => { const quote = liveQuotes[item.symbol] || (!realDataMode ? stocks[item.symbol] : null) || item; const hasQuote = Number.isFinite(quote.price); return <div className={selectedSymbol === item.symbol ? "watch-row selected" : "watch-row"} key={item.symbol}><button className="watch-row-main" onClick={() => selectSymbol(item.symbol)}><span><strong>{item.name}</strong><small>{item.symbol}</small></span><span className={quote.change >= 0 ? "quote up" : "quote down"}><strong>{hasQuote ? quote.price.toFixed(2) : "—"}</strong><small>{hasQuote ? `${quote.change >= 0 ? "+" : ""}${quote.change.toFixed(2)}%` : realDataMode ? "尚未查询" : "预览模式"}</small></span></button><button className="watch-remove" aria-label="移除自选" onClick={() => { void removeWatchlist(item.symbol); }}><X size={12} /></button></div>; })}</section>)}</div>
    <div className="sidebar-status"><span className="status-dot" />{realDataMode ? "Pi / QVeris 实时检查已启用" : "等待配置真实数据"}</div>
