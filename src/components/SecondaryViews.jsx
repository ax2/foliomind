...
export function PortfolioView() {
  ...
  return <div className="secondary-page portfolio-page"><header><div><h1>投资组合</h1><p>持仓、市值与未实现盈亏</p></div><div className="page-header-actions"><button className="secondary-button" onClick={() => { void generateReview(); }} disabled={!positions.length}><CalendarDots size={17} />生成复盘</button><button className="secondary-button" onClick={exportReport} disabled={!positions.length}><DownloadSimple size={17} />导出报告</button><button className="secondary-button" onClick={() => portfolioImportInput.current?.click()} disabled={importBusy}><UploadSimple size={17} />{importBusy ? "导入中…" : "导入持仓"}</button><button className="primary-action" onClick={openCreate}><Plus size={17} />添加持仓</button></div></header>
    ...
    <section className="risk-overview" aria-label="组合风险洞察">
      ...
      {positions.length === 0 ? <p className="risk-empty">添加持仓并获取真实行情后，这里会显示集中度与数据覆盖情况。</p> : <><div className="risk-metrics"><article><span>最大持仓</span><strong>{risk.topPosition?.name || "—"}</strong><small>{risk.topWeight == null ? "等待真实行情" : `${formatPercent(risk.topWeight)} 组合占比`}</small></article><article><span>行情覆盖</span><strong>{risk.pricedCoverage == null ? "—" : formatPercent(risk.pricedCoverage)}</strong><small>{metrics.pricedCount}/{metrics.totalCount} 个持仓</small></article><article><span>未计价成本</span><strong>{risk.missingCostWeight == null ? "—" : formatPercent(risk.missingCostWeight)}</strong><small>{risk.missingCostWeight == null ? "等待真实行情" : "暂未纳入市值计算"}</small></article></div><div className="risk-metrics risk-history-metrics"><article><span>加权波动（样本）</span><strong>{risk.weightedVolatility == null ? "—" : `${risk.weightedVolatility.toFixed(2)}%`}</strong><small>{risk.weightedVolatility == null ? "至少需要 3 个历史收益点" : `${risk.historicalSampleCount} 个历史收益点 · 未年化`}</small></article><article><span>平均相关性</span><strong>{risk.averageCorrelation == null ? "—" : risk.averageCorrelation.toFixed(2)}</strong><small>{risk.correlationPairs ? `${risk.correlationPairs} 组持仓重叠计算` : "至少需要两组重叠序列"}</small></article><article><span>历史数据覆盖</span><strong>{risk.historicalCoverage == null ? "—" : formatPercent(risk.historicalCoverage)}</strong><small>{risk.historicalCount ? `${risk.historicalCount} 个持仓可计算` : "暂无可计算历史"}</small></article></div><div className="risk-signal-list">{risk.signals.length ? risk.signals.map((signal) => <article className={`risk-signal ${signal.level}`} key={`${signal.level}-${signal.title}`}><span>{signal.level === "critical" ? <Warning size={17} /> : signal.level === "warning" ? <Warning size={17} /> : <Info size={17} />}</span><div><strong>{signal.title}</strong><p>{signal.detail}</p></div></article>) : <p className="risk-empty">当前没有可确认的风险信号；补齐历史行情后才会计算波动率与相关性。</p>}</div></>}
    </section>
