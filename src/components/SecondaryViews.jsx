...
const portfolioFormDefaults = { symbol: "", name: "", market: "", quantity: "", averageCost: "", takeProfitPrice: "", stopLossPrice: "" };
...
export function PortfolioView() {
  ...
  const openEdit = (position) => {
    setEditing(position);
    setError("");
    setForm({ symbol: position.symbol, name: position.name, market: position.market, quantity: String(position.quantity), averageCost: String(position.averageCost), takeProfitPrice: position.takeProfitPrice == null ? "" : String(position.takeProfitPrice), stopLossPrice: position.stopLossPrice == null ? "" : String(position.stopLossPrice) });
    setDialogOpen(true);
  };
  ...
  return <div className="secondary-page portfolio-page"><header><div><h1>投资组合</h1><p>持仓、市值与未实现盈亏</p></div><div className="page-header-actions"><button className="secondary-button" onClick={exportReport} disabled={!positions.length}><DownloadSimple size={17} />导出报告</button><button className="primary-action" onClick={openCreate}><Plus size={17} />添加持仓</button></div></header>
    ...
    {positions.length === 0 ? <div className="empty-state portfolio-empty"><Briefcase size={30} /><strong>还没有持仓</strong><p>添加持仓后，这里会汇总真实行情与盈亏。</p><button className="primary-action" onClick={openCreate}><Plus size={16} />添加第一笔持仓</button></div> : <section className="portfolio-table" aria-label="持仓明细"><div className="portfolio-table-head"><span>标的</span><span>数量</span><span>成本</span><span>现价</span><span>市值</span><span>未实现盈亏</span><span>占比</span><span>止盈 / 止损</span><span>操作</span></div>{metrics.rows.map((row) => <div className="portfolio-row" key={row.id}><span><strong>{row.name}</strong><small>{row.symbol}{row.market ? ` · ${row.market}` : ""}</small></span><span>{row.quantity}</span><span>{money(row.averageCost)}</span><span>{money(row.currentPrice)}</span><span>{money(row.marketValue)}</span><span className={row.pnl == null ? "" : row.pnl >= 0 ? "up" : "down"}>{money(row.pnl)}{row.pnlPercent == null ? "" : ` (${formatPercent(row.pnlPercent)})`}</span><span>{row.weight == null ? "—" : formatPercent(row.weight)}</span><span className="portfolio-alert-plan">{row.takeProfitPrice == null && row.stopLossPrice == null ? "未设置" : <>{row.takeProfitPrice != null && <small className={row.takeProfitTriggered ? "triggered" : ""}>止盈 {money(row.takeProfitPrice)}{row.takeProfitTriggered ? " · 已触发" : ""}</small>}{row.stopLossPrice != null && <small className={row.stopLossTriggered ? "triggered stop" : "stop"}>止损 {money(row.stopLossPrice)}{row.stopLossTriggered ? " · 已触发" : ""}</small>}</>}</span><span className="portfolio-actions"><button className="icon-button" aria-label={`编辑${row.symbol}持仓`} onClick={() => openEdit(row)}>编辑</button><button className="icon-button" aria-label={`删除${row.symbol}持仓`} onClick={() => { void deletePosition(row); }}>删除</button></span></div>)}</section>}
    {dialogOpen && <div className="modal-backdrop" role="presentation"><form className="modal-card portfolio-modal" onSubmit={submit}><div className="modal-heading"><h2>{editing ? "编辑持仓" : "添加持仓"}</h2><button type="button" className="icon-button" aria-label="关闭" onClick={() => setDialogOpen(false)}><X size={18} /></button></div><p className="modal-help">保存后会使用真实行情计算市值与未实现盈亏；达到提醒价只通知你，不会自动下单。</p><label>标的<select aria-label="持仓标的" value={form.symbol} onChange={(event) => selectSymbol(event.target.value)} required>{!form.symbol && <option value="">请选择标的</option>}{watchlist.map((item) => <option key={item.symbol} value={item.symbol}>{item.name}（{item.symbol}）</option>)}{editing && !watchlist.some((item) => item.symbol === editing.symbol) && <option value={editing.symbol}>{editing.name}（{editing.symbol}）</option>}</select></label><label>持仓数量<input aria-label="持仓数量" type="number" min="0.0001" step="any" value={form.quantity} onChange={(event) => setForm((value) => ({ ...value, quantity: event.target.value }))} required /></label><label>平均成本<input aria-label="平均成本" type="number" min="0.01" step="0.01" value={form.averageCost} onChange={(event) => setForm((value) => ({ ...value, averageCost: event.target.value }))} required /></label><div className="portfolio-alert-fields"><label>止盈价（可选）<input aria-label="止盈价" type="number" min="0.01" step="0.01" placeholder="达到后提醒" value={form.takeProfitPrice} onChange={(event) => setForm((value) => ({ ...value, takeProfitPrice: event.target.value }))} /></label><label>止损价（可选）<input aria-label="止损价" type="number" min="0.01" step="0.01" placeholder="跌破后提醒" value={form.stopLossPrice} onChange={(event) => setForm((value) => ({ ...value, stopLossPrice: event.target.value }))} /></label></div>{error && <p className="form-error" role="alert">{error}</p>}<button className="primary-action" type="submit" disabled={busy}>{busy ? "保存中…" : "保存持仓"}</button></form></div>}
  </div>;
}
