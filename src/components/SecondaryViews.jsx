...
import { activeResearchFilterCount, DEFAULT_RESEARCH_FILTERS, filterResearchItems, normalizeResearchFilters, researchResultsCsv, RESEARCH_FILTER_FIELDS, RESEARCH_SORT_OPTIONS, sortResearchItems } from "../lib/research.js";
...
function ConditionEditor({ condition, index, onChange, onRemove, canRemove }) {
  ...
  return <div className="condition-row">
    ...
    {type.valueType === "select" ? <select aria-label={`条件${index + 1}取值`} value={condition.value} onChange={(event) => onChange({ ...condition, value: event.target.value })}>{type.options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : <label className="condition-value"><input aria-label={`条件${index + 1}数值`} type="number" min={type.id === "price_level" ? "0.000001" : undefined} step={type.id === "capital_flow" ? "1000000" : "0.1"} value={condition.value} onChange={(event) => onChange({ ...condition, value: event.target.value })} /><span>{type.unit}</span></label>}
...
export function NotificationsView() {
  ...
  const clearReadNotifications = useLabStore((state) => state.clearReadNotifications);
  const selectSymbol = useLabStore((state) => state.selectSymbol);
  const setActiveView = useLabStore((state) => state.setActiveView);
  const [query, setQuery] = useState("");
  ...
  const exportCsv = () => {
    const blob = new Blob([notificationCsv(filteredNotifications)], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `foliomind-notifications-${new Date().toISOString().slice(0, 10)}.csv`;
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
    window.setTimeout(() => URL.revokeObjectURL(url), 0);
...
export function SettingsView() {
  ...
  return <div className="secondary-page settings-page" aria-busy={loadState === "loading" || busy || runtimeConfiguring}><header><div><h1>设置</h1><p>真实数据、模型网关与本地凭据</p></div><span>{environmentLabel}</span></header>
    ...
    <section className="settings-card"><div className="settings-card-title"><div><strong>金融数据能力</strong><small>默认连接金融数据能力目录；首次固化后按稳定 tool schema 调用，避免每次重新搜索。</small></div><span className="status-pill ok">CAP</span></div><label>数据能力 API<input value={form.capabilityBaseUrl} disabled={formDisabled} onChange={(event) => { setForm((value) => ({ ...value, capabilityBaseUrl: event.target.value })); setConnectionTestState("idle"); setConnectionTestMessage(""); }} aria-label="数据能力 API" /></label><div className="settings-inline"><label className="refresh-policy-select">数据渠道<select aria-label="数据渠道" value={form.dataChannel || "qveris-cap"} disabled={formDisabled} onChange={(event) => { setForm((value) => ({ ...value, dataChannel: event.target.value })); setConnectionTestState("idle"); setConnectionTestMessage(""); }}>{DATA_CHANNEL_OPTIONS.map((channel) => <option value={channel.id} key={channel.id}>{channel.label}</option>)}</select></label><span className="settings-inline-note">{DATA_CHANNEL_OPTIONS.find((channel) => channel.id === (form.dataChannel || "qveris-cap"))?.description || "兼容 CAP 的稳定工具契约"}</span></div><label>Provider ID<input value={form.dataProvider || "qveris_finance"} disabled={formDisabled} onChange={(event) => { setForm((value) => ({ ...value, dataProvider: event.target.value })); setConnectionTestState("idle"); setConnectionTestMessage(""); }} aria-label="Provider ID" /></label><div className="settings-inline-note">渠道与 Provider 会写入本地配置，并参与缓存隔离；当前内置工具使用稳定的金融 tool schema。</div><div className="connection-test-row"><button type="button" className="secondary-button" disabled={formDisabled || !status.credentialConfigured || status.demo || capabilityEndpointChanged || connectionTestState === "loading"} onClick={() => { void testDataConnection(); }}>{connectionTestState === "loading" ? "测试中…" : "测试数据连接"}</button><span className={`connection-test-status ${connectionTestState}`} role="status">{connectionTestMessage || (capabilityEndpointChanged ? "保存并应用新地址后可测试" : status.credentialConfigured ? "使用当前自选的第一个标的进行真实 CAP 测试" : "保存 API Key 后可测试真实数据")}</span></div></section>
