...
function marketColumnValue(item, quote, column) {
  ...
  if (column === "asOf") return { value: quote?.asOf ? formatQuoteFreshness(quote.asOf) : "—", className: "quote-source" };
  return { value: "—" };
}

function LiveDataState({ state, receivedCount = 0, totalCount = 0, onRetry, onSettings, compact = false }) {
  const copy = liveDataStateCopy(state, { receivedCount, totalCount });
  const actionLabel = copy.action === "settings" ? "去设置" : copy.action === "retry" ? "立即重试" : "";
  const onAction = copy.action === "settings" ? onSettings : copy.action === "retry" ? onRetry : undefined;
...
export function MarketView() {
  ...
  return <div className="secondary-page"><header><div><h1>市场行情</h1><p>跨市场指数、自选与异动概览</p></div><div className="market-header-actions"><div className="market-view-controls" aria-label="行情视图"><label><span>视图</span><select aria-label="行情视图" value={activeMarketViewId} onChange={(event) => selectMarketView(event.target.value)}>{activeMarketViewId === CUSTOM_MARKET_VIEW_ID && <option value={CUSTOM_MARKET_VIEW_ID}>临时视图</option>}{marketViews.map((view) => <option value={view.id} key={view.id}>{view.name}</option>)}</select></label><button className="secondary-button" type="button" onClick={() => { setViewSaveOpen(true); setViewNotice(""); }}>保存视图</button>{activeMarketViewId.startsWith("custom-") && <button className="icon-button" type="button" aria-label="删除当前行情视图" onClick={deleteMarketView}>删除</button>}</div><div className="market-column-menu"><button className="secondary-button" type="button" aria-expanded={columnsOpen} aria-haspopup="true" onClick={() => setColumnsOpen((value) => !value)}><List size={16} />列设置</button>{columnsOpen && <div className="market-column-popover" role="group" aria-label="自选行情列设置"><strong>自选行情列</strong><small>只显示已返回的真实字段；空字段保持“—”。</small>{MARKET_COLUMN_DEFINITIONS.map((column) => <label key={column.id}><input type="checkbox" checked={marketColumns.includes(column.id)} disabled={marketColumns.includes(column.id) && marketColumns.length === 1} onChange={() => toggleMarketColumn(column.id)} />{column.label}</label>)}<button type="button" className="notification-link" onClick={() => setColumnsAndView(DEFAULT_MARKET_COLUMNS)}>恢复默认列</button></div>}</div><button className="secondary-button" disabled={liveDataLoading} onClick={realDataMode ? retry : openSettings}><ArrowsClockwise size={16} />{realDataMode ? liveDataLoading ? "更新中…" : "刷新真实数据" : "配置数据"}</button></div></header>
    ...
    <div className="index-board" aria-label="自选行情概览">{returnedQuotes.length > 0 ? returnedQuotes.map((item) => { const quote = liveQuotes[item.symbol]; const freshness = quoteFreshness(quote.asOf); return <article className="index-board-item" role="button" tabIndex="0" aria-label={`打开${item.name}详情`} onClick={(event) => openMarketSymbol(event, item.symbol)} onKeyDown={(event) => openMarketSymbol(event, item.symbol)} key={item.symbol}><span>{item.name} <small>{item.symbol}</small></span><strong>{formatPrice(quote.price)}</strong><small className={changeToneClass(quote.change)}>{formatPercent(quote.change)}</small><em className={`quote-source quote-source-${freshness.state}`}>{quote.source || "数据服务"} · {formatQuoteFreshness(quote.asOf)}</em></article>; }) : <LiveDataState state={dataState} receivedCount={0} totalCount={watchlist.length} onRetry={retry} onSettings={openSettings} />}</div>
    {returnedQuotes.length > 0 ? <section className="market-breadth" aria-label="自选市场宽度"><div className="market-breadth-heading"><div><h2>自选市场宽度</h2><p>仅统计已返回真实价格和涨跌幅的标的；缺失字段不会参与计算。</p></div><span>{breadth.pricedCount}/{breadth.totalCount} 有行情</span></div><div className="market-breadth-grid"><article><span>上涨</span><strong className="up">{breadth.upCount}</strong><small>有涨跌幅的标的</small></article><article><span>下跌</span><strong className="down">{breadth.downCount}</strong><small>有涨跌幅的标的</small></article><article><span>平盘</span><strong>{breadth.flatCount}</strong><small>{breadth.changeCount ? `共 ${breadth.changeCount} 个可比较` : "涨跌幅暂无"}</small></article><article><span>最大涨幅</span>{breadth.topGainer ? <><strong className="up">{formatPercent(breadth.topGainer.change)}</strong><small>{breadth.topGainer.name} · {breadth.topGainer.symbol}</small></> : <><strong>—</strong><small>暂无可比较数据</small></>}</article><article><span>最大跌幅</span>{breadth.topLoser ? <><strong className="down">{formatPercent(breadth.topLoser.change)}</strong><small>{breadth.topLoser.name} · {breadth.topLoser.symbol}</small></> : <><strong>—</strong><small>暂无可比较数据</small></>}</article></div>{breadth.missingCount > 0 ? <p className="market-breadth-note">另有 {breadth.missingCount} 个标的暂未返回有效价格，保持为空并会在下一次刷新时重试。</p> : null}</section> : null}
    ...
    <section className="market-table"><div className="market-table-heading"><h2>我的自选</h2><small>{visibleColumns.length} 个数据列 · 点击标的可打开详情</small></div><div className="table-head" style={{ "--market-table-columns": `minmax(250px, 1fr) repeat(${visibleColumns.length}, minmax(120px, 1fr)) 120px` }}><span>名称 / 代码</span>{visibleColumns.map((column) => <span key={column.id}>{column.label}</span>)}<span>市场</span></div>{watchlist.map((item) => { const quote = liveQuotes[item.symbol]; return <div className="table-row market-table-row" role="button" tabIndex="0" aria-label={`打开${item.name}详情`} onClick={(event) => openMarketSymbol(event, item.symbol)} onKeyDown={(event) => openMarketSymbol(event, item.symbol)} key={item.symbol} style={{ "--market-table-columns": `minmax(250px, 1fr) repeat(${visibleColumns.length}, minmax(120px, 1fr)) 120px` }}><span><strong>{item.name}</strong><small>{item.symbol}</small></span>{visibleColumns.map((column) => { const cell = marketColumnValue(item, quote, column.id); return <span className={cell.className || ""} key={`${item.symbol}-${column.id}`}>{cell.value}</span>; })}<span>{item.market}</span></div>; })}</section>
...
export function MonitorView() {
  ...
  const createRule = async (event) => { event.preventDefault(); await addRule({ scope: form.scope, symbol: form.scope === "watchlist" ? "*" : form.symbol, strategyId: selectedStrategy.id, conditions: normalizeConditions(form.conditions, selectedStrategy.id), logic: form.logic, threshold: Number(form.conditions[0]?.value) || selectedStrategy.defaultThreshold, intervalSeconds: Number(form.intervalSeconds) }); setDialogOpen(false); };
...
export function SettingsView() {
  ...
  const saveKey = () => run(async () => { await saveQVerisCredential(apiKey); const next = { ...status, credentialConfigured: true, keyPrefix: apiKeyPrefix(apiKey) }; setApiKey(""); setStatus(next); setIntegrationStatus(next); setConnectionTestState("idle"); setConnectionTestMessage(""); }, "数据服务密钥已保存");
  const clearKey = () => run(async () => { await clearQVerisCredential(); const next = { ...status, credentialConfigured: false, keyPrefix: "" }; setStatus(next); setIntegrationStatus(next); setConnectionTestState("idle"); setConnectionTestMessage(""); }, "数据服务密钥已清除");
  ...
  const testDataConnection = async () => {
    ...
    try {
      ...
      setConnectionTestMessage(`连接成功 · ${symbol} 已返回真实行情 · ${source} · ${formatQuoteFreshness(asOf)} · ${elapsed}ms`);
    } catch (error) {
      setConnectionTestState("error");
