...
金融查询首次成功后，Web Host 会按数据类型、渠道地址和模型隔离保存工具选择与参数模板；后续请求优先使用内置 `foliomind_data` 直接 Call，缓存失效再回退到一次 Discover。QVeris 和模型网关的瞬时 408/425/429/5xx 失败会经过有界指数退避重试，并尊重上游 `Retry-After`；取消信号会打断退避等待。自选行情刷新使用受限并发（默认 2，localhost 开发面板可调至 1–4），避免多个标的串行等待，也不会无限制地冲击上游服务。前端轮询仅在页面可见时执行，进入后台会跳过请求，回到前台或窗口重新获得焦点时立即触发一次刷新。凭据、模型或渠道变更会使相关缓存失效。
